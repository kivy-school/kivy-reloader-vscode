...
export class KivyHotReloader {
     ...
     */
    async sendFullProject(projectPath: string, workspaceRoot: string): Promise<boolean> {
        try {
     ...
     */
    async sendUpdate(files: string[], workspaceRoot: string): Promise<boolean> {
        try {
     ...
     */
    private async createDeltaPackage(files: string[], workspaceRoot: string): Promise<Buffer> {
        const zip = new JSZip();
     ...
     */
    private async sendToServer(zipBuffer: Buffer): Promise<boolean> {
    ...
    startWatching(workspaceRoot: string): void {
        ...
        const handleChange = async (uri: vscode.Uri) => {
            ...
            const autoReload = config.get<boolean>('hotReload.autoSend', false);
     ...
     */
    private async createFullPackage(files: string[], projectPath: string, workspaceRoot: string): Promise<Buffer> {
        const zip = new JSZip();
